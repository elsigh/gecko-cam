...
describe("KV2 cache consistency", () => {
    ...
    describe("read-your-writes after put", () => {
        ...
        it("metadata is consistent with value after update", async (ctx) => {
            ...
            const first = await kv.get("meta-sync");
            if (first.exists)
                await first.value; // Populate cache
            await kv.set("meta-sync", "new", { createdBy: "bob", version: 2 });
            const result = await kv.get("meta-sync");
            expect(result.exists).toBe(true);
            if (result.exists) {
    ...
    describe("read-your-writes after delete", () => {
        it("immediately sees not found after delete (not stale cache)", async (ctx) => {
            ...
            const first = await kv.get("to-delete");
            expect(first.exists).toBe(true);
            if (first.exists) {
        ...
        it("can recreate key after delete", async (ctx) => {
            ...
            const first = await kv.get("recreate");
            if (first.exists)
                await first.value;
            ...
            const result = await kv.get("recreate");
            expect(result.exists).toBe(true);
            if (result.exists) {
    ...
    describe("cache isolation between keys", () => {
        it("updating one key does not affect another key's cached value", async (ctx) => {
            ...
            const a1 = await kv.get("key-a");
            const b1 = await kv.get("key-b");
            if (a1.exists)
                await a1.value;
            if (b1.exists)
                await b1.value;
        ...
        it("deleting one key does not affect another key", async (ctx) => {
            ...
            // Populate cache
            const keep1 = await kv.get("keep");
            const remove1 = await kv.get("remove");
            if (keep1.exists)
                await keep1.value;
            if (remove1.exists)
                await remove1.value;
    ...
    describe("stream consistency", () => {
        it("stream reflects current value after update", async (ctx) => {
            const { kv } = ctx;
            await kv.set("stream-key", { version: 1 }, { createdBy: "test", version: 1 });
            const first = await kv.get("stream-key");
            if (first.exists)
                await first.value; // Populate cache
            await kv.set("stream-key", { version: 2 }, { createdBy: "test", version: 2 });
        ...
        it("value and stream are consistent with each other", async (ctx) => {
            ...
            const result = await kv.get("consistent");
            expect(result.exists).toBe(true);
            if (result.exists) {
...
//# sourceMappingURL=cached-kv.cache.test.js.map