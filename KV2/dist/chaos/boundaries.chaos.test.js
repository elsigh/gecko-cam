...
import { FakeBlobStore, KV2, afterEach, beforeEach, it, uniqueTestPrefix, } from "./chaos-test-setup.js";
/**
...
describe("Chaos: Boundaries", () => {
    ...
    describe("large value threshold boundaries", () => {
        it("should handle value exactly at threshold", async (ctx) => {
            ...
            const result = await kv.get("exact");
        ...
        it("should handle value one byte under threshold", async (ctx) => {
            ...
            const result = await kv.get("under");
            expect(result.exists).toBe(true);
            if (result.exists) {
        ...
        it("should handle value one byte over threshold", async (ctx) => {
            ...
            const result = await kv.get("over");
            expect(result.exists).toBe(true);
            if (result.exists) {
        ...
        it("should correctly choose encoding at boundary for binary", async (ctx) => {
            ...
            const resultUnder = await kv.get("binary-under");
            const resultOver = await kv.get("binary-over");
        ...
        it("should handle threshold of 1", async (ctx) => {
            ...
            const result = await kv.get("tiny");
            expect(result.exists).toBe(true);
            if (result.exists) {
        ...
        it("should handle threshold of 0", async (ctx) => {
            ...
            const result = await kv.get("zero-threshold");
            expect(result.exists).toBe(true);
            if (result.exists) {
        ...
        it("should handle very large threshold", async (ctx) => {
            ...
            const result = await kv.get("huge-threshold");
            expect(result.exists).toBe(true);
            if (result.exists) {
    ...
    describe("key boundaries", () => {
        it("should reject empty key", async (ctx) => {
            ...
            await expect(kv.set("", "empty-key-value", { createdBy: "test", version: 1 })).rejects.toThrow("Key cannot be empty");
        ...
        it("should handle key with newlines and tabs", async (ctx) => {
            ...
            const result = await kv.get(whitespaceKey);
            expect(result.exists).toBe(true);
            if (result.exists) {
    ...
    describe("value type boundaries", () => {
        it("should handle null value", async (ctx) => {
            ...
            const result = await kv.get("null-value");
            expect(result.exists).toBe(true);
            if (result.exists) {
        ...
        it("should reject undefined value", async (ctx) => {
            ...
            try {
                ...
                await kv.set("undefined-value", undefined, {
                    createdBy: "test",
                    version: 1,
                });
            }
        ...
        it("should handle boolean values", async (ctx) => {
            ...
            const trueResult = await kv.get("true");
            const falseResult = await kv.get("false");
        ...
        it("should handle number edge cases", async (ctx) => {
            ...
            for (let i = 0; i < numbers.length; i++) {
                const result = await kv.get(`num-${i}`);
                expect(result.exists).toBe(true);
                if (result.exists) {
        ...
        it("should handle arrays with holes (sparse arrays)", async (ctx) => {
            ...
            const result = await kv.get("sparse");
            expect(result.exists).toBe(true);
            if (result.exists) {
                const value = await result.value;
        ...
        it("should handle object with symbol keys (symbols are ignored in JSON)", async (ctx) => {
            ...
            const result = await kv.get("symbol-obj");
            expect(result.exists).toBe(true);
            if (result.exists) {
                const value = await result.value;
        ...
        it("should handle Date objects (become strings in JSON)", async (ctx) => {
            ...
            const result = await kv.get("date");
            expect(result.exists).toBe(true);
            if (result.exists) {
                const value = await result.value;
        ...
        it("should handle BigInt (throws in JSON.stringify)", async (ctx) => {
            const { prefix, blobStore } = ctx;
            const kv = new KV2({ prefix, blobStore });
            await expect(kv.set("bigint", BigInt(12345), { createdBy: "test", version: 1 })).rejects.toThrow();
        });
        it("should handle circular reference detection", async (ctx) => {
            ...
            await expect(kv.set("circular", circular, { createdBy: "test", version: 1 })).rejects.toThrow();
        });
    });
    describe("metadata boundaries", () => {
        it("should handle empty metadata object", async (ctx) => {
            ...
            const result = await kv.get("empty-meta");
            expect(result.exists).toBe(true);
            if (result.exists) {
        ...
        it("should handle metadata with many fields", async (ctx) => {
            ...
            const result = await kv.get("big-meta");
            expect(result.exists).toBe(true);
            if (result.exists) {
        ...
        it("should handle metadata with deeply nested structure", async (ctx) => {
            ...
            const result = await kv.get("deep-meta");
            expect(result.exists).toBe(true);
            if (result.exists) {
                expect(result.metadata).toEqual(meta);
            }
        ...
        it("should handle metadata with unicode", async (ctx) => {
            ...
            const result = await kv.get("unicode-meta");
            expect(result.exists).toBe(true);
            if (result.exists) {
                expect(result.metadata).toEqual(meta);
            }
    ...
    describe("header length boundaries", () => {
        it("should handle header that approaches 4-byte length limit", async (ctx) => {
            ...
            const result = await kv.get("large-header");
            expect(result.exists).toBe(true);
            if (result.exists) {
        ...
        it("should handle minimal header", async (ctx) => {
            ...
            const result = await kv.get("minimal");
            expect(result.exists).toBe(true);
            if (result.exists) {
    ...
    describe("prefix boundaries", () => {
        ...
        it("should handle deeply nested prefix", async (ctx) => {
            const { blobStore } = ctx;
            const deepPrefix = "a/b/c/d/e/f/g/h/i/j/k/";
            const kv = new KV2({
                prefix: deepPrefix,
                blobStore,
            });
            await kv.set("deep-key", "deep-value", { createdBy: "test", version: 1 });
            const result = await kv.get("deep-key");
            expect(result.exists).toBe(true);
            if (result.exists) {
        ...
        it("should handle keys that look like the prefix", async (ctx) => {
            ...
            const result = await kv.get("test-prefix/nested");
            expect(result.exists).toBe(true);
    ...
    describe("concurrent threshold changes", () => {
        it("should handle reading values written with different thresholds", async (ctx) => {
            ...
            const kvLarge = new KV2({
                prefix,
                blobStore,
                largeValueThreshold: 1000000,
            });
            const result = await kvLarge.get("cross-threshold");
            expect(result.exists).toBe(true);
            if (result.exists) {
    ...
    describe("ArrayBuffer and typed array boundaries", () => {
        it("should handle ArrayBuffer values", async (ctx) => {
            ...
            const result = await kv.get("arraybuffer");
            expect(result.exists).toBe(true);
            if (result.exists) {
                const value = await result.value;
        ...
        it("should handle empty ArrayBuffer", async (ctx) => {
            ...
            const result = await kv.get("empty-arraybuffer");
            expect(result.exists).toBe(true);
            if (result.exists) {
                const value = await result.value;
        ...
        it("should handle large ArrayBuffer", async (ctx) => {
            ...
            const result = await kv.get("large-arraybuffer");
            expect(result.exists).toBe(true);
            if (result.exists) {
                const value = await result.value;
...
//# sourceMappingURL=boundaries.chaos.test.js.map