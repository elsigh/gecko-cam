...
function createStores(kv) {
    ...
    const sessionsKV = kv.getStore("session/").withIndexes({
        byUserId: { key: (s) => s.userId },
    });
    const apiKeysKV = kv.getStore("apikey/").withIndexes({
        byUserId: { key: (k) => k.userId },
    });
    const auditKV = kv.getStore("audit/");
...
describe("Auth patterns", () => {
    ...
    it("duplicate email throws KVIndexConflictError", async () => {
        ...
        expect(error.indexKey).toBe("taken@example.com");
    });
    ...
    it("duplicate username throws KVIndexConflictError", async () => {
        ...
        expect(error).toBeInstanceOf(KVIndexConflictError);
        expect(error.indexName).toBe("byUsername");
        expect(error.indexKey).toBe("alice");
    });
    ...
    it("audit log: append entries and query by user prefix", async () => {
        ...
        const user1Entries = [];
        for await (const [key, entry] of auditKV.entries("user-1/")) {
        ...
        const user2Entries = [];
        for await (const [key, entry] of auditKV.entries("user-2/")) {
...
//# sourceMappingURL=auth-patterns.test.js.map