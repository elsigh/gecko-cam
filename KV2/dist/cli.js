...
function consumeOption(name, fallback) {
    const idx = args.indexOf(name);
...
const branch = consumeOption("--branch", process.env.VERCEL_GIT_COMMIT_REF || "main");
...
// ---------------------------------------------------------------------------
async function cmdKeys(prefixArg) {
    if (allKeys) {
        let count = 0;
    ...
    if (result.cursor) {
        process.stderr.write(`${dim(`${result.keys.length} key(s) (more available, use --all or --limit)`)}\n`);
    }
    else {
...
async function cmdGet(key) {
    ...
    if (verbose) {
        ...
        if (result.metadata !== undefined) {
            process.stderr.write(`${dim(`metadata: ${JSON.stringify(result.metadata)}`)}\n`);
        }
    }
...
async function cmdSet(key, jsonValue, metadataJson) {
    ...
    if (!allowWrites) {
        process.stderr.write(`${red("Write operations require --allow-writes flag")}\n`);
        process.exitCode = 1;
        return;
    }
    let value;
    ...
    if (metadataJson) {
        try {
            ...
            process.stderr.write(`${red(`Invalid JSON metadata: ${metadataJson}`)}\n`);
            process.exitCode = 1;
            return;
        }
    }
...
// Command dispatch
// ---------------------------------------------------------------------------
async function dispatch(cmd, cmdArgs) {
    switch (cmd) {
        case "keys":
...
async function repl() {
    ...
    process.stderr.write(`${dim(`  writes:  ${allowWrites ? "enabled" : "disabled"}`)}\n`);
    process.stderr.write(`${dim('  Type "help" for commands, Ctrl+C to exit.')}\n\n`);
    while (true) {
        let line;
        try {
