...
describe("KV2", () => {
    ...
    describe("get", () => {
        ...
        it("provides stream access to raw bytes", async (ctx) => {
            const { kv } = ctx;
            await kv.set("stream-test", { data: "test" }, { createdBy: "test", version: 1 });
    ...
    describe("put", () => {
        ...
        it("stores objects as JSON", async (ctx) => {
            ...
            const result = await kv.get("obj-key");
            expect(result.exists).toBe(true);
            if (result.exists) {
        ...
        it("stores boolean values", async (ctx) => {
            ...
            const resultTrue = await kv.get("bool-true");
            const resultFalse = await kv.get("bool-false");
        ...
        it("stores array values", async (ctx) => {
            ...
            const result = await kv.get("array");
            expect(result.exists).toBe(true);
            if (result.exists) {
    ...
    describe("getMany", () => {
        it("returns entries for existing keys", async (ctx) => {
            ...
            const results = await kv.getMany(["gm-a", "gm-b", "gm-missing"]);
    ...
    describe("entries().page()", () => {
        it("returns paginated entries", async (ctx) => {
            ...
            const page1 = await kv.entries().page(3);
            expect(page1.entries.length).toBe(3);
            expect(page1.cursor).toBeDefined();
            const page2 = await kv.entries().page(3, page1.cursor);
    ...
    describe("delete", () => {
        ...
        it("invalidates cache on delete", async (ctx) => {
            ...
            // Populate cache
            const first = await kv.get("cache-delete");
            if (first.exists)
                await first.value;
            // Delete
...
describe("KV2 large values (streaming from blob)", () => {
    ...
    it("set and get large JSON (object > 16 bytes) — read .value", async () => {
        ...
        const result = await kv.get("large-json");
        expect(result.exists).toBe(true);
        if (result.exists) {
    ...
    it("large value: read .value then .stream uses buffered data", async () => {
        ...
        const result = await kv.get("stream-after-value");
        expect(result.exists).toBe(true);
        if (result.exists) {
    ...
    it("large value: consuming stream then reading value throws", async () => {
        ...
        if (result.exists) {
            ...
            try {
                ...
                threw = true;
                expect(e.message).toContain("stream has been consumed");
            }
            expect(threw).toBe(true);
        }
    });
    ...
    it("entry.update() on blob result (not from cache)", async () => {
        ...
        const result = await kv.get("update-blob");
        expect(result.exists).toBe(true);
        if (result.exists) {
            await result.update("updated");
            const updated = await kv.get("update-blob");
...
//# sourceMappingURL=cached-kv.test.js.map