...
describe("TypedKV (getStore)", () => {
    ...
    describe("basic operations", () => {
        it("creates typed sub-KV with prefix", async (ctx) => {
            ...
            await users.set("123", { name: "Alice", email: "alice@example.com" }, { createdBy: "admin", version: 1 });
        ...
        it("delete removes key", async (ctx) => {
            const { kv } = ctx;
            const users = kv.getStore("users/");
            await users.set("to-delete", { name: "Bob", email: "bob@example.com" }, { createdBy: "admin", version: 1 });
        ...
        it("overwrites existing key", async (ctx) => {
            const { kv } = ctx;
            const users = kv.getStore("users/");
            await users.set("user1", { name: "Old Name", email: "old@example.com" }, { createdBy: "admin", version: 1 });
            await users.set("user1", { name: "New Name", email: "new@example.com" }, { createdBy: "admin", version: 2 });
    ...
    describe("list operations", () => {
        it("list returns keys without sub-prefix", async (ctx) => {
            const { kv } = ctx;
            const users = kv.getStore("users/");
            await users.set("1", { name: "A", email: "a@test.com" }, { createdBy: "test", version: 1 });
            await users.set("2", { name: "B", email: "b@test.com" }, { createdBy: "test", version: 1 });
            const keys = [];
        ...
        it("list with prefix filters within sub-store", async (ctx) => {
            const { kv } = ctx;
            const users = kv.getStore("users/");
            await users.set("admin/1", { name: "Admin1", email: "admin1@test.com" }, { createdBy: "test", version: 1 });
            await users.set("admin/2", { name: "Admin2", email: "admin2@test.com" }, { createdBy: "test", version: 1 });
            await users.set("regular/1", { name: "Regular1", email: "regular@test.com" }, { createdBy: "test", version: 1 });
            const keys = [];
    ...
    describe("isolation", () => {
        it("different stores are isolated", async (ctx) => {
            ...
            await users.set("1", { name: "Alice", email: "alice@test.com" }, { createdBy: "test", version: 1 });
            await posts.set("1", { title: "Hello", content: "World", authorId: "1" }, { createdBy: "test", version: 1 });
            const userKeys = [];
            for await (const key of users.keys()) {
                userKeys.push(key);
            }
    ...
    describe("nested sub-stores", () => {
        it("supports nested getStore calls", async (ctx) => {
            const { kv } = ctx;
            const tenantA = kv.getStore("tenants/a/");
            const tenantB = kv.getStore("tenants/b/");
            await tenantA.set("user1", { name: "Alice", email: "alice@a.com" }, { createdBy: "test", version: 1 });
            await tenantB.set("user1", { name: "Bob", email: "bob@b.com" }, { createdBy: "test", version: 1 });
    ...
    describe("stream access", () => {
        it("provides stream access through sub-store", async (ctx) => {
            const { kv } = ctx;
            const users = kv.getStore("users/");
            await users.set("stream-test", { name: "Test", email: "test@example.com" }, { createdBy: "test", version: 1 });
    ...
    describe("keysShallow", () => {
        it("returns only direct children without /", async (ctx) => {
            const { kv } = ctx;
            const store = kv.getStore("items/");
            await store.set("top-level", { name: "A", email: "a@test.com" }, { createdBy: "test", version: 1 });
            await store.set("nested/child", { name: "B", email: "b@test.com" }, { createdBy: "test", version: 1 });
            await store.set("another", { name: "C", email: "c@test.com" }, { createdBy: "test", version: 1 });
            const keys = [];
    ...
    describe("getMany", () => {
        it("fetches multiple typed keys with prefix stripping", async (ctx) => {
            ...
            const users = kv.getStore("users/");
            await users.set("1", { name: "Alice", email: "alice@test.com" }, { createdBy: "test", version: 1 });
            await users.set("2", { name: "Bob", email: "bob@test.com" }, { createdBy: "test", version: 1 });
    ...
    describe("keys().page() and entries().page()", () => {
        it("returns paginated keys", async (ctx) => {
            const { kv } = ctx;
            const store = kv.getStore("pg/");
            for (let i = 0; i < 5; i++) {
                await store.set(`k${i}`, { name: `User${i}`, email: `u${i}@test.com` }, { createdBy: "test", version: 1 });
            }
        ...
        it("returns paginated entries", async (ctx) => {
            const { kv } = ctx;
            const store = kv.getStore("epg/");
            for (let i = 0; i < 5; i++) {
                await store.set(`e${i}`, { name: `User${i}`, email: `u${i}@test.com` }, { createdBy: "test", version: 1 });
            }
    ...
    describe("reindex error", () => {
        it("throws when no indexes defined", async (ctx) => {
            const { kv } = ctx;
            const store = kv.getStore("no-idx/");
            let threw = false;
            try {
                await store.reindex();
            }
                ...
                threw = true;
                expect(e.message).toContain("No indexes defined");
            }
            expect(threw).toBe(true);
        });
    });
    describe("entries", () => {
        it("returns entries with stripped prefix", async (ctx) => {
            ...
            const users = kv.getStore("users/");
            await users.set("1", { name: "Alice", email: "alice@test.com" }, { createdBy: "test", version: 1 });
            await users.set("2", { name: "Bob", email: "bob@test.com" }, { createdBy: "test", version: 2 });
            const entries = [];
...
describe("TypedKV unique index keys/entries queries", () => {
    setupTestContext();
    function createIndexedStore(ctx) {
        return ctx.kv.getStore("uq/").withIndexes({
    ...
    it("keys() on unique index returns key via iterator", async (ctx) => {
        ...
        await store.set("p1", { title: "Hello", slug: "hello", status: "draft", tags: [] }, { createdBy: "test", version: 1 });
        const keys = [];
...
describe("TypedKV validation errors", () => {
    ...
    it("keys() throws for unknown index", async (ctx) => {
        ...
        try {
            // biome-ignore lint/suspicious/noExplicitAny: testing validation
            for await (const _ of store.keys({ unknownIdx: "x" })) {
                // should not reach
            }
        }
        catch (e) {
    ...
    it("entries() throws for unknown index", async (ctx) => {
        ...
        try {
            // biome-ignore lint/suspicious/noExplicitAny: testing validation
            for await (const _ of store.entries({ unknownIdx: "x" })) {
                // should not reach
            }
        }
        catch (e) {
...
describe("TypedKV nested getStore", () => {
    ...
    it("creates sub-store from TypedKV with set/get", async (ctx) => {
        const parent = ctx.kv.getStore("parent/");
        const child = parent.getStore("child/");
        await child.set("u1", { name: "Alice", email: "alice@test.com" }, { createdBy: "test", version: 1 });
...
describe("TypedKV orphan cleanup via .page()", () => {
    ...
    function createStore(ctx) {
        return ctx.kv.getStore("orphan-pg/").withIndexes({
            bySlug: { key: (doc) => doc.slug, unique: true },
    ...
    function createPlainStore(ctx) {
        return ctx.kv.getStore("orphan-pg/");
    }
    it("non-unique keys().page() orphan: deleted main entry", async (ctx) => {
        const store = createStore(ctx);
        await store.set("p1", { title: "A", slug: "a", status: "draft", tags: [] }, { createdBy: "test", version: 1 });
        await store.set("p2", { title: "B", slug: "b", status: "draft", tags: [] }, { createdBy: "test", version: 1 });
    ...
    it("non-unique keys().page() orphan: stale value", async (ctx) => {
        const store = createStore(ctx);
        await store.set("p1", { title: "A", slug: "a1", status: "draft", tags: [] }, { createdBy: "test", version: 1 });
        // Change status bypassing index maintenance
        const plain = createPlainStore(ctx);
        await plain.set("p1", { title: "A", slug: "a1", status: "published", tags: [] }, { createdBy: "test", version: 2 });
        const { keys } = await store.keys({ byStatus: "draft" }).page(10);
        expect(keys).toEqual([]);
    });
    it("non-unique entries().page() orphan: deleted main entry", async (ctx) => {
        const store = createStore(ctx);
        await store.set("p1", { title: "A", slug: "a2", status: "draft", tags: [] }, { createdBy: "test", version: 1 });
        const plain = createPlainStore(ctx);
        await plain.delete("p1");
    ...
    it("non-unique entries().page() orphan: stale value", async (ctx) => {
        const store = createStore(ctx);
        await store.set("p1", { title: "A", slug: "a3", status: "draft", tags: [] }, { createdBy: "test", version: 1 });
        const plain = createPlainStore(ctx);
        await plain.set("p1", { title: "A", slug: "a3", status: "published", tags: [] }, { createdBy: "test", version: 2 });
        const { entries } = await store.entries({ byStatus: "draft" }).page(10);
        expect(entries).toEqual([]);
    });
...
//# sourceMappingURL=typed-kv.test.js.map