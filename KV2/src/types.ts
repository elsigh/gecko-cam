...
export class KVIndexConflictError extends Error {
  ...
  constructor(indexName: string, indexKey: string) {
    super(
      `Unique index "${indexName}" conflict: another entry already uses this index value`,
    );
 ...
 */
export interface EntriesIterable<V, M>
  extends AsyncIterable<[string, KVEntry<V, M>]> {
  /**
...
export interface KVLike<M> {
  ...
  ): import("./typed-kv.js").TypedKV<V, SubM, I>;
}
