...
// ============================================================================
// 1. MAX_HEADER_SIZE streaming read check
// ============================================================================

interface InjectableTestContext extends BaseTestContext {
  blobStore: InjectableBlobStore;
  kv: KV2<unknown>;
  prefix: PrefixString;
}

type InjectableTestFn = (ctx: InjectableTestContext) => Promise<void> | void;

const injectableIt = (name: string, fn: InjectableTestFn): void => {
  baseIt(name, fn as (ctx: BaseTestContext) => Promise<void> | void);
};

const injectableBeforeEach = (
  fn: (ctx: InjectableTestContext) => Promise<void> | void,
): void => {
  baseBeforeEach(fn as (ctx: BaseTestContext) => Promise<void> | void);
};

const injectableAfterEach = (
  fn: (ctx: InjectableTestContext) => Promise<void> | void,
): void => {
...
describe("Security: MAX_HEADER_SIZE streaming read check", () => {
  injectableBeforeEach((ctx) => {
    ...
    ctx.kv = new KV2<unknown>({ prefix: ctx.prefix, blobStore: ctx.blobStore });
  });
  ...
  injectableIt(
    "rejects blob with header length exceeding MAX_HEADER_SIZE",
    async (ctx) => {
      ...
      await expect(kv.get("oversized-header")).rejects.toThrow(
  ...
  injectableIt("rejects blob with header length at 0xFFFFFFFF", async (ctx) => {
    ...
    await expect(kv.get("max-uint32-header")).rejects.toThrow(
      /exceeds maximum allowed size/,
    );
  });

  injectableIt(
    "accepts blob with header length within MAX_HEADER_SIZE",
    async (ctx) => {
      const { kv, blobStore, prefix } = ctx;
      ...
      const result = await kv.get<string>("ok-header");
...
describe("Security: Key validation", () => {
  ...
  describe("get rejects invalid keys", () => {
    ...
    baseIt("rejects key with null byte", async () => {
      const { kv } = createTestKV();
      await expect(kv.get("foo\x00bar")).rejects.toThrow(
        "Key cannot contain null bytes",
      );
    });
    ...
    baseIt("rejects key exceeding max length", async () => {
      ...
      await expect(kv.get(longKey)).rejects.toThrow(
  ...
  describe("set rejects invalid keys", () => {
    ...
    baseIt("rejects key with null byte", async () => {
      const { kv } = createTestKV();
      await expect(kv.set("key\x00null", "value")).rejects.toThrow(
        "Key cannot contain null bytes",
      );
    });
    ...
    baseIt("rejects key exceeding max length", async () => {
      const { kv } = createTestKV();
      await expect(kv.set("x".repeat(2049), "value")).rejects.toThrow(
        "Key exceeds maximum length",
      );
    });
  });
  ...
  describe("delete rejects invalid keys", () => {
    ...
    baseIt("rejects key with null byte", async () => {
      const { kv } = createTestKV();
      await expect(kv.delete("foo\x00")).rejects.toThrow(
        "Key cannot contain null bytes",
      );
    });
  });
...
// ============================================================================
// 3. Index key encoding (non-unique index separator safety)
// ============================================================================
...
describe("Security: Index key encoding", () => {
  ...
  function createUserStore(ctx: {
    kv: import("./cached-kv.js").KV2<TestMeta>;
  }) {
    return ctx.kv.getStore<TestUser, TestMeta>("users/").withIndexes({
      byRole: { key: (u) => u.role },
  ...
  it("handles slash in non-unique index value without collision", async (ctx) => {
    ...
    await store.set(
      "user1",
      { name: "Alice", role: "admin/super", category: "cat-a" },
      { createdBy: "test", version: 1 },
    );
    await store.set(
      "user2",
      { name: "Bob", role: "admin", category: "cat-b" },
      { createdBy: "test", version: 1 },
    );
  ...
  it("handles slash in primary key without collision", async (ctx) => {
    ...
    await store.set(
      "org/team/alice",
      { name: "Alice", role: "admin", category: "cat-a" },
      { createdBy: "test", version: 1 },
    );
    await store.set(
      "org/team/bob",
  ...
  it("prevents collision between similar index/pk combinations", async (ctx) => {
    ...
    await store.set(
      "c",
      { name: "User1", role: "a/b", category: "cat-1" },
      { createdBy: "test", version: 1 },
    );
    await store.set(
      "b/c",
      { name: "User2", role: "a", category: "cat-2" },
      { createdBy: "test", version: 1 },
    );
  ...
  it("handles percent in index value (double-encoding safety)", async (ctx) => {
    ...
    await store.set(
      "user1",
      { name: "Alice", role: "100%", category: "cat-a" },
      { createdBy: "test", version: 1 },
    );
    await store.set(
      "user2",
      { name: "Bob", role: "100%2F", category: "cat-b" },
      { createdBy: "test", version: 1 },
    );
  ...
  it("entries query works with slashes in index values", async (ctx) => {
    ...
    await store.set(
      "user1",
      { name: "Alice", role: "dept/eng", category: "cat-a" },
      { createdBy: "test", version: 1 },
    );
  ...
  it("reindex works correctly with slashes in index values", async (ctx) => {
    ...
    await store.set(
      "user1",
      { name: "Alice", role: "admin/level-2", category: "cat-a" },
      { createdBy: "test", version: 1 },
    );
...
describe("Security: Unique index CAS protection", () => {
  ...
  it("rejects duplicate unique index value", async (ctx) => {
    const store = ctx.kv
      .getStore<{ email: string }, TestMeta>("emails/")
      .withIndexes({
  ...
  it("allows updating same key with same unique value", async (ctx) => {
    const store = ctx.kv
      .getStore<{ email: string; name: string }, TestMeta>("emails/")
      .withIndexes({
        byEmail: { key: (u) => u.email, unique: true },
      });
  ...
  it("unique index uses override:false for new entries (CAS)", async (ctx) => {
    ...
    const store = ctx.kv
      .getStore<{ email: string }, TestMeta>("cas/")
      .withIndexes({
        byEmail: { key: (u) => u.email, unique: true },
      });
    ...
    expect((err as KVIndexConflictError).indexName).toBe("byEmail");
    expect((err as KVIndexConflictError).indexKey).toBe("shared@test.com");
  });
});
...
describe("Security: KVIndexConflictError message redaction", () => {
  baseIt(
    "error message does not contain the conflicting index value",
    async () => {
  ...
  it("thrown error from store does not leak index value", async (ctx) => {
    const store = ctx.kv
      .getStore<{ email: string }, TestMeta>("leak/")
      .withIndexes({
        byEmail: { key: (u) => u.email, unique: true },
      });
    ...
    try {
      ...
      const err = e as KVIndexConflictError;
