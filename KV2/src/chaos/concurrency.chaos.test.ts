...
describe("Chaos: Concurrency", () => {
  ...
  describe("parallel writes to same key", () => {
    it("should handle rapid-fire writes to the same key", async (ctx) => {
      ...
      const result = await kv.get<{ index: number; data: string }>(key);
    ...
    it("should handle interleaved reads and writes", async (ctx) => {
      ...
      const reads = results.filter(
        (r) => r != null && typeof r === "object" && "exists" in r,
    ...
    it("should handle parallel deletes with reads", async (ctx) => {
      ...
      await Promise.all(
        keys.map((key) =>
          kv.set(key, "to-be-deleted", { createdBy: "setup", version: 1 }),
        ),
      );
  ...
  describe("parallel operations on different keys", () => {
    it("should handle massive parallel writes to different keys", async (ctx) => {
      ...
      const writes = keys.map((key, i) =>
        kv.set(key, { id: i }, { createdBy: "bulk", version: 1 }),
      );
      ...
      const reads = await Promise.all(
        keys.map(async (key, i) => {
          const result = await kv.get<{ id: number }>(key);
    ...
    it("should handle parallel reads without corrupting each other", async (ctx) => {
      ...
      const reads = Array.from({ length: 100 }, () =>
        kv.get<typeof complexValue>("shared-read"),
      );
  ...
  describe("lazy value access races", () => {
    it("should handle multiple simultaneous accesses to lazy value", async (ctx) => {
      ...
      await kv.set(
        "lazy-target",
        { data: "test" },
        { createdBy: "setup", version: 1 },
      );

      const result = await kv.get<{ data: string }>("lazy-target");
      expect(result.exists).toBe(true);
      if (result.exists) {
    ...
    it("should handle concurrent value and stream access on same result", async (ctx) => {
      ...
      const result = await kv.get<Buffer>("stream-race");
      expect(result.exists).toBe(true);
      if (result.exists) {
  ...
  describe("write-then-read consistency", () => {
    it("should read the written value immediately after write", async (ctx) => {
      ...
      for (let i = 0; i < iterations; i++) {
        ...
        const result = await kv.get<typeof value>(key);
        expect(result.exists).toBe(true);
        if (result.exists) {
    ...
    it("should handle update-then-read cycles", async (ctx) => {
      ...
      for (let i = 0; i < 30; i++) {
        ...
        const result = await kv.get<{ count: number }>(key);
        expect(result.exists).toBe(true);
        if (result.exists) {
          const value = await result.value;
  ...
  describe("multiple KV instances", () => {
    it("should handle multiple KV instances on same prefix", async (ctx) => {
      ...
      const kv1 = new KV2<TestMetadata>({ prefix, blobStore });
      const kv2 = new KV2<TestMetadata>({ prefix, blobStore });
      const kv3 = new KV2<TestMetadata>({ prefix, blobStore });
      ...
      const result3 = await kv3.get<string>("shared-key");
    ...
    it("should handle parallel operations across multiple instances", async (ctx) => {
      ...
      const instances = Array.from(
        { length: 5 },
        () => new KV2<TestMetadata>({ prefix, blobStore }),
      );

      // Each instance writes to its own keys
      const writes = instances.flatMap((instance, i) =>
        Array.from({ length: 10 }, (_, j) =>
          instance.set(`instance-${i}-key-${j}`, `value-${i}-${j}`, {
      ...
      for (const instance of instances) {
        for (let i = 0; i < 5; i++) {
          for (let j = 0; j < 10; j++) {
            const result = await instance.get<string>(`instance-${i}-key-${j}`);
            expect(result.exists).toBe(true);
            if (result.exists) {
    ...
    it("should handle iteration while keys are being added", async (ctx) => {
      ...
      await Promise.all(
        Array.from({ length: 10 }, (_, i) =>
          kv.set(`existing-${i}`, i, { createdBy: "setup", version: 1 }),
        ),
      );
    ...
    it("should handle iteration while keys are being deleted", async (ctx) => {
      ...
      await Promise.all(
        Array.from({ length: 20 }, (_, i) =>
          kv.set(`deletable-${i}`, i, { createdBy: "setup", version: 1 }),
        ),
      );
    ...
    it("should survive burst of operations", async (ctx) => {
      ...
      for (let i = 0; i < burstSize; i++) {
        ...
        switch (op) {
          case 0:
            operations.push(
              kv.set(key, { i }, { createdBy: "burst", version: i }),
            );
            break;
          case 1:
            ...
            operations.push(
              (async () => {
                ...
                for await (const k of kv.keys()) {
                  keys.push(k);
                  if (keys.length > 10) break; // Limit iteration
                }
                return keys;
              })(),
            );
