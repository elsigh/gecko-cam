...
import { type TestResult, runTests } from "./vitest-compat.js";
 ...
 */
function extractImportedTests(indexContent: string): Set<string> {
 ...
 */
function checkAllTestsImported(): void {
  ...
  if (missingTests.length > 0) {
    ...
    console.error(
      "The following test files are not imported in test-index.ts:",
    );
...
const results = await runTests({
  ...
  onKeepAlive: (testName: string, elapsedMs: number) => {
    ...
    console.log(
      `\x1b[33m⏳ [${elapsedSec}s] Still running: ${testName}\x1b[0m`,
    );
  },
  onProgress: (result: TestResult) => {
