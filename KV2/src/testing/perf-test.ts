...
export async function runPerfTest(iterations = 10): Promise<PerfResult> {
  const prefix = uniqueTestPrefix();
  const blobStore = new VercelBlobStore();
  const kv = new KV2<{ test: boolean }>({ prefix, blobStore });
  ...
  if (warmup.exists) await warmup.value;
  ...
  // Calculate stats
  const rawBlobAvg =
    rawBlobReads.reduce((a, b) => a + b, 0) / rawBlobReads.length;
