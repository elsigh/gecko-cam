...
function createTestKV(options?: { largeValueThreshold?: number }) {
...
export function registerCoreTests(t: TestingPrimitives): void {
  const { describe, it, expect, beforeEach, afterEach } = t;

  let blobStore: FakeBlobStore;
  let kv: KV2<TestMetadata>;
  ...
  describe("KV2", () => {
    beforeEach(() => {
      ...
      kv = new KV2<TestMetadata>({ prefix, blobStore });
    });
    ...
    describe("set", () => {
      ...
      it("stores JSON objects", async () => {
        ...
        const result = await kv.get<typeof obj>("object");
        expect(result.exists).toBe(true);
        if (result.exists) {
      ...
      it("stores boolean values", async () => {
        ...
        const trueResult = await kv.get<boolean>("bool-true");
        const falseResult = await kv.get<boolean>("bool-false");

        expect(trueResult.exists).toBe(true);
        expect(falseResult.exists).toBe(true);
        if (trueResult.exists) expect(await trueResult.value).toBe(true);
        if (falseResult.exists) expect(await falseResult.value).toBe(false);
      });
    });
...
export function registerBinaryTests(t: TestingPrimitives): void {
  const { describe, it, expect, beforeEach, afterEach } = t;
  ...
  describe("KV2 binary values", () => {
    ...
    it("stores and retrieves Buffer values", async () => {
      ...
      const result = await kv.get<Buffer>("buffer");
      expect(result.exists).toBe(true);
      if (result.exists) {
    ...
    it("stores and retrieves Uint8Array values", async () => {
      ...
      const result = await kv.get<Buffer>("uint8");
      expect(result.exists).toBe(true);
      if (result.exists) {
        const value = await result.value;
...
export function registerLargeValueTests(t: TestingPrimitives): void {
  const { describe, it, expect, beforeEach, afterEach } = t;
  ...
  describe("KV2 large values", () => {
    ...
    it("stores and retrieves large string values", async () => {
      ...
      const result = await kv.get<string>("large-string");
      expect(result.exists).toBe(true);
      if (result.exists) {
    ...
    it("stores and retrieves large JSON objects", async () => {
      ...
      const result = await kv.get<typeof largeObject>("large-object");
      expect(result.exists).toBe(true);
      if (result.exists) {
...
export function registerTypedKVTests(t: TestingPrimitives): void {
  const { describe, it, expect, beforeEach, afterEach } = t;
  ...
  describe("TypedKV (getStore)", () => {
    ...
    it("isolates keys between stores", async () => {
      const store1 = kv.getStore<string>("store1/");
      const store2 = kv.getStore<string>("store2/");
      ...
      if (r1.exists) expect(await r1.value).toBe("value1");
      if (r2.exists) expect(await r2.value).toBe("value2");
    });
  });
}

export function registerStreamingTests(t: TestingPrimitives): void {
  const { describe, it, expect, beforeEach, afterEach } = t;
  ...
  describe("KV2 streaming input", () => {
    ...
    it("handles multi-chunk stream", async () => {
      ...
      const result = await kv.get<Buffer>("multi-chunk");
      expect(result.exists).toBe(true);
      if (result.exists) {
        const value = await result.value;
...
export function registerStressTests(t: TestingPrimitives): void {
  ...
  describe("Stress tests", () => {
    it(`handles ${STRESS_COUNT} concurrent writes`, async () => {
      ...
      const writes = Array.from({ length: STRESS_COUNT }, (_, i) =>
        kv.set(`key-${i}`, { index: i }, { createdBy: "stress", version: i }),
      );

      await Promise.all(writes);

      for (let i = 0; i < STRESS_COUNT; i++) {
        const result = await kv.get<{ index: number }>(`key-${i}`);
        expect(result.exists).toBe(true);
        if (result.exists) {
    ...
    it(`handles ${STRESS_COUNT} concurrent reads`, async () => {
      ...
      for (let i = 0; i < STRESS_COUNT; i++) {
        await kv.set(
          `read-${i}`,
          { index: i },
          { createdBy: "stress", version: 1 },
        );
      }

      const reads = Array.from({ length: STRESS_COUNT }, (_, i) =>
        kv.get<{ index: number }>(`read-${i}`),
      );
    ...
    it("handles rapid read-after-write", async () => {
      ...
      for (let i = 0; i < 10; i++) {
        await kv.set(
          "rapid",
          { iteration: i },
          { createdBy: "test", version: i },
        );
        const result = await kv.get<{ iteration: number }>("rapid");
        expect(result.exists).toBe(true);
        if (result.exists) {
...
export function registerAllTests(t: TestingPrimitives): void {
  registerCoreTests(t);
