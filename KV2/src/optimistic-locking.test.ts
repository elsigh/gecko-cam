...
async function expectVersionConflictError(
...
describe("Optimistic Locking", () => {
  ...
  describe("version field", () => {
    ...
    it("version changes after update", async (ctx) => {
      const { kv } = ctx;
      const result1 = await kv.set(
        "test-key",
        { value: 1 },
        { createdBy: "test", version: 1 },
      );
      const result2 = await kv.set(
        "test-key",
        { value: 2 },
  ...
  describe("entry.update()", () => {
    it("entry.update() succeeds when version matches", async (ctx) => {
      ...
      const entry = await kv.get<{ value: number }>("update-test");
      ...
      if (entry.exists) {
        ...
        const updated = await kv.get<{ value: number }>("update-test");
    ...
    it("entry.update() fails with KVVersionConflictError when version changed", async (ctx) => {
      ...
      const entry = await kv.get<{ value: number }>("conflict-test");
      expect(entry.exists).toBe(true);
      if (entry.exists) {
    ...
    it("entry.update() preserves metadata by default", async (ctx) => {
      ...
      await kv.set(
        "metadata-test",
        { value: 1 },
        { createdBy: "original", version: 1 },
      );

      const entry = await kv.get<{ value: number }>("metadata-test");
      expect(entry.exists).toBe(true);
      if (entry.exists) {
        // Update without providing metadata
        await entry.update({ value: 2 });

        const updated = await kv.get<{ value: number }>("metadata-test");
        expect(updated.exists).toBe(true);
        if (updated.exists) {
    ...
    it("entry.update() can change metadata", async (ctx) => {
      ...
      const entry = await kv.get<{ value: number }>("metadata-update");
      expect(entry.exists).toBe(true);
      if (entry.exists) {
        await entry.update({ value: 2 }, { createdBy: "updated", version: 2 });

        const updated = await kv.get<{ value: number }>("metadata-update");
        expect(updated.exists).toBe(true);
        if (updated.exists) {
  ...
  describe("set with expectedVersion", () => {
    ...
    it("set with expectedVersion fails when mismatched", async (ctx) => {
      ...
      await expectVersionConflictError(
        kv.set(
          "mismatch-test",
          { value: 2 },
          { createdBy: "test", version: 2 },
  ...
  describe("override option", () => {
    ...
    it("set with override: true (default) overwrites existing", async (ctx) => {
      ...
      const entry = await kv.get<{ value: number }>("overwrite-test");
      expect(entry.exists).toBe(true);
      if (entry.exists) {
  ...
  describe("concurrent operations", () => {
    it("concurrent updates: one wins, one gets conflict error", async (ctx) => {
      ...
      const entry1 = await kv.get<{ value: number }>("concurrent-test");
      const entry2 = await kv.get<{ value: number }>("concurrent-test");
    ...
    it("read-modify-write pattern with retry on conflict", async (ctx) => {
      ...
      const incrementWithRetry = async (): Promise<number> => {
        ...
        while (attempts < maxAttempts) {
          attempts++;
          const entry = await kv.get<{ counter: number }>("retry-test");
  ...
  describe("TypedKV integration", () => {
    it("TypedKV entry.update() works with prefixed keys", async (ctx) => {
      const { kv } = ctx;
      const typedKV = new TypedKV<
        { value: number },
        { createdBy: string; version: number }
      >(kv, "typed/");
    ...
    it("TypedKV set options pass through correctly", async (ctx) => {
      ...
      const { version } = await typedKV.set(
        "versioned",
        { value: 1 },
        { createdBy: "test", version: 1 },
      );

      const result = await typedKV.set(
        "versioned",
        { value: 2 },
        { createdBy: "test", version: 2 },
        { expectedVersion: version },
    ...
    it("TypedKV version conflict uses unprefixed key in error message", async (ctx) => {
      ...
      expect((error as Error).message).toContain("typed/my-key");
    });
  });
});
