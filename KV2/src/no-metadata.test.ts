...
interface NoMetadataTestContext {
  kv: KV2;
...
describe("KV2 without metadata", () => {
  beforeEach((ctx) => {
    const testCtx = ctx as unknown as NoMetadataTestContext;
    if (useRealBlobStore()) {
      const result = createTestKV<undefined>({
  ...
  afterEach(async (ctx) => {
    const { cleanup } = ctx as unknown as NoMetadataTestContext;
    await cleanup();
  });

  describe("set without metadata", () => {
    it("can set value without metadata argument", async (ctx) => {
      const { kv } = ctx as unknown as NoMetadataTestContext;
    ...
    it("can set value with undefined metadata", async (ctx) => {
      ...
      const result = await kv.get<string>("key2");
      expect(result.exists).toBe(true);
      if (result.exists) {
    ...
    it("can set JSON objects without metadata", async (ctx) => {
      ...
      const result = await kv.get<User>("user1");
      expect(result.exists).toBe(true);
      if (result.exists) {
    ...
    it("can set binary data without metadata", async (ctx) => {
      ...
      const result = await kv.get<Buffer>("binary1");
      expect(result.exists).toBe(true);
      if (result.exists) {
        const value = await result.value;
  ...
  describe("stream without metadata", () => {
    it("can stream value set without metadata", async (ctx) => {
      ...
      const result = await kv.get<typeof data>("stream-key");
      expect(result.exists).toBe(true);
      if (result.exists) {
