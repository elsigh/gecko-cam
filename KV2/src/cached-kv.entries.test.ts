...
describe("KV2 entries", () => {
  ...
  it("returns all entries", async (ctx) => {
    ...
    const entries: Array<
      [string, unknown, { createdBy: string; version: number }]
    > = [];
    for await (const [key, entry] of kv.entries<string>()) {
  ...
  it("returns entries matching prefix", async (ctx) => {
    ...
    for await (const [key, entry] of kv.entries<{ name: string }>("users/")) {
  ...
  it("respects custom concurrency parameter", async (ctx) => {
    ...
    const entries: Array<[string, unknown]> = [];
    for await (const [key, entry] of kv.entries<string>(undefined, 1)) {
      entries.push([key, await entry.value]);
    }
  ...
  it("includes metadata in entries", async (ctx) => {
    ...
    await kv.set(
      "test-key",
      { data: "test" },
      { createdBy: "alice", version: 42 },
    );

    for await (const [key, entry] of kv.entries<{ data: string }>()) {
  ...
  it("provides working stream access", async (ctx) => {
    ...
    for await (const [, entry] of kv.entries<string>()) {
...
describe("KV2 entries with prefix option", () => {
  it("works with global prefix", async () => {
    ...
    const entries: Array<[string, unknown]> = [];
    for await (const [key, entry] of prefixedKv.entries<string>()) {
      entries.push([key, await entry.value]);
    }
...
describe("KV2 entries pagination", () => {
  ...
  it("page returns first page of entries", async (ctx) => {
    ...
    const { entries, cursor } = await kv.entries<string>().page(2);
  ...
  it("page returns all entries when limit exceeds count", async (ctx) => {
    ...
    const { entries, cursor } = await kv.entries<string>().page(10);
  ...
  it("page with prefix filters entries", async (ctx) => {
    ...
    const { entries } = await kv.entries<{ name: string }>("users/").page(10);
  ...
  it("page includes metadata", async (ctx) => {
    ...
    const { entries } = await kv.entries<string>().page(10);
  ...
  it("page paginates through all entries with cursor", async (ctx) => {
    ...
    // Fetch in pages of 3
    do {
      const result = await kv.entries<string>().page(3, cursor);
