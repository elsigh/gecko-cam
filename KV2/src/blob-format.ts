 ...
 */
export function isPureJsonFormat(buffer: Buffer): boolean {
 ...
 */
export function parseBlob<M>(buffer: Buffer): ParsedBlob<M> {
  if (buffer.length === 0) {
  ...
  if (isPureJsonFormat(buffer)) {
    ...
    const header = JSON.parse(buffer.toString("utf-8")) as StoredEntry<M>;
  ...
  if (buffer.length < HEADER_LENGTH_BYTES) {
    throw new Error(
      `Buffer too small for binary format: ${buffer.length} bytes`,
    );
  }
  ...
  if (headerEnd > buffer.length) {
    throw new RangeError(
      `Header length ${headerLength} exceeds buffer size ${buffer.length}`,
    );
  }
  ...
  const hasPayload =
    header.encoding === "raw-json" || header.encoding === "raw-binary";
...
export function createBlob<M>(
  ...
  if (hasPayload) {
    ...
    if (headerBuffer.length >= MAX_HEADER_SIZE) {
      throw new Error(
        `Header too large: ${headerBuffer.length} bytes (max ${MAX_HEADER_SIZE})`,
      );
    }
 ...
 */
export function parseHeader<M>(buffer: Buffer): StoredEntry<M> {
  return parseBlob<M>(buffer).header;
}

/**
 * Checks if a blob has a payload (large value stored after header).
 */
export function hasPayload<M>(header: StoredEntry<M>): boolean {
