  ...
  if (process.env.INTEGRATION_TEST === "1") {
    describe("data operations (integration)", () => {
      ...
      it("set writes a value", () => {
        const { stderr, exitCode } = run(
          ["--allow-writes", ...baseArgs, "set", testKey, '{"hello":"world"}'],
          tokenEnv,
        );
        expect(exitCode).toBe(0);
      ...
      it("get returns error for missing key", () => {
        const { stdout, stderr, exitCode } = run(
          [...baseArgs, "get", "nonexistent-key-xyz"],
          tokenEnv,
        );
        expect(exitCode).toBe(1);
        expect(stdout).toBe("");
      ...
      it("del deletes the key", () => {
        const { stderr, exitCode } = run(
          ["--allow-writes", ...baseArgs, "del", testKey],
          tokenEnv,
        );
        expect(exitCode).toBe(0);
