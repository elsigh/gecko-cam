...
describe("KV2 binary values", () => {
  ...
  it("stores and retrieves Uint8Array values", async (ctx) => {
    ...
    if (result.exists) {
      const value = await result.value;
      expect(Buffer.from(value).toString("hex")).toBe(
  ...
  it("stores and retrieves empty Buffer", async (ctx) => {
    ...
    const result = await kv.get<Buffer>("empty-buffer");
    expect(result.exists).toBe(true);
    if (result.exists) {
      const value = await result.value;
  ...
  it("stores and retrieves binary data with all byte values", async (ctx) => {
    ...
    const result = await kv.get<Buffer>("all-bytes");
    expect(result.exists).toBe(true);
    if (result.exists) {
  ...
  it("streams binary values", async (ctx) => {
    ...
    const result = await kv.get<Buffer>("stream-binary");
    expect(result.exists).toBe(true);
    if (result.exists) {
...
describe("KV2 large values", () => {
  it("stores and retrieves large string values", async () => {
    ...
    const result = await largeKv.get<string>("large");
    expect(result.exists).toBe(true);
    if (result.exists) {
  ...
  it("stores and retrieves large JSON objects", async () => {
    ...
    const result = await largeKv.get<typeof largeObj>("large-obj");
    expect(result.exists).toBe(true);
    if (result.exists) {
  ...
  it("streams large values", async () => {
    ...
    const result = await largeKv.get<Buffer>("stream-large");
    expect(result.exists).toBe(true);
    if (result.exists) {
      const stream = await result.stream;
      const reader = stream.getReader();
...
describe("KV2 streaming input", () => {
  ...
  it("streaming input always uses large file mode", async (ctx) => {
    ...
    const result = await kv.get<Buffer>("small-stream");
    expect(result.exists).toBe(true);
    if (result.exists) {
...
describe("KV2 edge cases", () => {
  ...
  it("handles empty string value", async (ctx) => {
    ...
    const result = await kv.get<string>("empty-string");
    expect(result.exists).toBe(true);
    if (result.exists) {
  ...
  it("handles values with unicode characters", async (ctx) => {
    ...
    const result = await kv.get<string>("unicode-value");
    expect(result.exists).toBe(true);
    if (result.exists) {
  ...
  it("handles metadata with special characters", async (ctx) => {
    ...
    const result = await kv.get<string>("meta-test");
    expect(result.exists).toBe(true);
    if (result.exists) {
  ...
  it("handles deeply nested objects", async (ctx) => {
    ...
    const result = await kv.get<typeof deepObj>("deep");
    expect(result.exists).toBe(true);
    if (result.exists) {
  ...
  it("handles object with many keys", async (ctx) => {
    ...
    const result = await kv.get<typeof manyKeys>("many-keys");
    expect(result.exists).toBe(true);
    if (result.exists) {
