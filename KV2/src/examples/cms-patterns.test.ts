...
// --- Domain types (mirrors examples/cms/lib/types.ts) ---
...
function encodeUrlKey(url: string): string {
...
function createStores(kv: KV2<unknown>) {
  ...
  const historyKV: TypedKV<VersionHistoryEntry, unknown> =
    kv.getStore<VersionHistoryEntry>("history/");
  const usersKV = kv.getStore<User>("user/").withIndexes({
    byUsername: { key: (user) => user.username, unique: true },
  });
  const sessionsKV: TypedKV<Session, unknown> =
    kv.getStore<Session>("session/");
...
describe("CMS patterns", () => {
  ...
  it("update document with optimistic locking + archive to history store", async () => {
    ...
    // Archive to history
    await historyKV.set(
      historyKey("page", "doc-1", current.metadata.version),
      {
        document: oldDoc,
  ...
  it("concurrent update triggers version conflict", async () => {
    ...
    await entry1.update(
      { ...val1, title: "V2 by user1" },
      { ...entry1.metadata, updatedBy: "u1", version: 2 },
    );
    ...
    try {
      await entry2.update(
        { ...val2, title: "V2 by user2" },
        { ...entry2.metadata, updatedBy: "u2", version: 2 },
      );
    } catch (e) {
      error = e as Error;
    }
  ...
  it("restore document from history", async () => {
    ...
    await documentsKV.set(
      "page/doc-1",
      { ...originalDoc, title: "Changed", content: "Changed content." },
      { ...originalMeta, updatedAt: 2000, version: 2 },
    );
  ...
  it("delete document, verify indexes cleaned", async () => {
    ...
    // Slug index gone
    expect((await documentsKV.get({ bySlug: "to-delete" })).exists).toBe(
      false,
    );

    // URL indexes gone
    expect(
      (await documentsKV.get({ byUrl: encodeUrlKey("/to-delete") })).exists,
    ).toBe(false);
    expect(
      (await documentsKV.get({ byUrl: encodeUrlKey("/remove-me") })).exists,
    ).toBe(false);
  ...
  it("slug conflict on create (unique index)", async () => {
    ...
    expect((error as KVIndexConflictError).indexName).toBe("bySlug");
    expect((error as KVIndexConflictError).indexKey).toBe("taken-slug");
  });
  ...
  it("change slug on update: old index removed, new index works", async () => {
    ...
    // Old slug gone
    expect((await documentsKV.get({ bySlug: "old-slug" })).exists).toBe(
      false,
    );
    ...
    expect(
      (await documentsKV.get({ byUrl: encodeUrlKey("/old-slug") })).exists,
    ).toBe(false);
