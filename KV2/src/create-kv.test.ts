...
} from "./testing/create-kv-test-setup.js";
...
describe("createKV", () => {
  ...
  describe("environment detection", () => {
    ...
    it("defaults to development/main when env vars missing", ({
      blobStore,
      cache,
    }) => {
  ...
  describe("branch encoding", () => {
    it("encodes slashes in branch names", async ({ blobStore, cache }) => {
      ...
      const hasEncodedBranch = blobs.blobs.some((b) =>
        b.pathname.includes("feature%2Fmy-feature"),
      );
    ...
    it("lowercases branch names", async ({ blobStore, cache }) => {
      ...
      const blobs = await blobStore.list({ prefix: "cached-kv/preview/" });
      const hasLowercaseBranch = blobs.blobs.some((b) =>
        b.pathname.includes("feature-branch"),
      );
  ...
  describe("long branch names", () => {
    it("truncates branch name and adds hash suffix for >64 chars", async ({
      blobStore,
      cache,
    }) => {
  ...
  describe("local dev fallback", () => {
    it("falls back to DiskBlobStore when no token and not on Vercel", async ({
      originalEnv,
    }) => {
      // biome-ignore lint/performance/noDelete: process.env coerces undefined to string "undefined"
    ...
    it("throws when no token on Vercel", ({ originalEnv }) => {
      ...
      try {
        ...
        threw = true;
        expect((e as Error).message).toContain("Missing BLOB_READ_WRITE_TOKEN");
      }
