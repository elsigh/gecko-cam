...
function createTmpDir(): string {
  return path.join(
    os.tmpdir(),
    `disk-blob-test-${Date.now()}-${Math.random().toString(36).slice(2, 8)}`,
  );
}
  ...
  describe("basic operations", () => {
    it("put and get a value", async () => {
      ...
      try {
        ...
        const result = getResult as NonNullable<typeof getResult>;
  ...
  describe("list", () => {
    ...
    it("filters by prefix", async () => {
      ...
      try {
        ...
        expect(result.blobs.every((b) => b.pathname.startsWith("users/"))).toBe(
          true,
        );
      } finally {
        await store.clear();
