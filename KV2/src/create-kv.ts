 ...
 * ```
 */
export function createKV<M = undefined>(options: CreateKVOptions = {}): KV2<M> {
 ...
 */
function resolveBlobStore(options: CreateKVOptions): BlobStore {
  if (options.blobStore) return options.blobStore;

  const token = options.token ?? process.env.BLOB_READ_WRITE_TOKEN;
  if (token) return new VercelBlobStore(token);

  // No token — are we deployed on Vercel?
  if (process.env.VERCEL) {
    throw new Error(
      "Missing BLOB_READ_WRITE_TOKEN. To get started:\n" +
        "\n" +
  ...
  if (!loggedFallback) {
    loggedFallback = true;
    console.info(
      `[KV2] No BLOB_READ_WRITE_TOKEN found, using local disk storage at ${cacheDir}`,
    );
  }
 ...
 */
function detectEnvironment(options: CreateKVOptions): {
  env: string;
  branch: string;
} {
  const env = options.env || process.env.VERCEL_ENV || "development";
  const rawBranch =
    options.branch || process.env.VERCEL_GIT_COMMIT_REF || "main";
 ...
 */
function buildPrefix(env: string, branch: string, userPrefix: string): string {
