...
describe("Index orphan handling", () => {
  ...
  function createStore(ctx: { kv: import("./cached-kv.js").KV2<TestMeta> }) {
    return ctx.kv.getStore<TestDoc, TestMeta>("doc/").withIndexes({
      bySlug: {
        key: (doc) => doc.slug,
        unique: true,
      },
  ...
  }): TypedKV<TestDoc, TestMeta> {
    return ctx.kv.getStore<TestDoc>("doc/");
  }
  ...
  describe("orphan type 1: index points to deleted main entry", () => {
    it("get() on unique index detects and cleans orphan", async (ctx) => {
      ...
      await store.set(
        "page/1",
        { title: "Ghost", slug: "ghost", status: "draft", tags: [] },
    ...
    it("keys() on non-unique index detects and cleans orphan", async (ctx) => {
      ...
      await store.set(
        "page/1",
        { title: "Ghost", slug: "ghost1", status: "draft", tags: [] },
        { createdBy: "test", version: 1 },
      );
      await store.set(
        "page/2",
        { title: "Alive", slug: "alive1", status: "draft", tags: [] },
        { createdBy: "test", version: 1 },
      );
    ...
    it("entries() on non-unique index detects and cleans orphan", async (ctx) => {
      ...
      await store.set(
        "page/1",
        { title: "Ghost", slug: "ghost2", status: "draft", tags: [] },
        { createdBy: "test", version: 1 },
      );
      ...
      const entries: Array<[string, TestDoc]> = [];
    ...
    it("page() on non-unique index detects and cleans orphan", async (ctx) => {
      ...
      await store.set(
        "page/1",
        { title: "Ghost", slug: "ghost3", status: "draft", tags: [] },
        { createdBy: "test", version: 1 },
      );
      await store.set(
        "page/2",
        { title: "Alive", slug: "alive2", status: "draft", tags: [] },
        { createdBy: "test", version: 1 },
      );
  ...
  describe("orphan type 2: index points to value with different index key", () => {
    ...
    it("keys() on non-unique index detects stale status", async (ctx) => {
      ...
      await store.set(
        "page/1",
        { title: "Post", slug: "post1", status: "draft", tags: [] },
        { createdBy: "test", version: 1 },
      );
    ...
    it("entries() on non-unique index detects stale value", async (ctx) => {
      ...
      await store.set(
        "page/1",
        { title: "Post", slug: "post2", status: "draft", tags: [] },
        { createdBy: "test", version: 1 },
      );
  ...
  describe("orphan from failed optimistic-locking update", () => {
    it("entry.update() version conflict preserves correct indexes", async (ctx) => {
      ...
      try {
        await result.update(
          { ...oldValue, slug: "post-v3", updatedAt: "2024-01-03" },
          { createdBy: "test", version: 3 },
        );
      } catch (e) {
        error = e as Error;
      }
