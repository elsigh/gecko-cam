...
interface CacheTestContext extends BaseTestContext {
  cache: KVCache;
  fakeCache: FakeCache;
  errors: ErrorLog[];
}

type TypedTestFn = (ctx: CacheTestContext) => Promise<void> | void;
type TypedHookFn = (ctx: CacheTestContext) => Promise<void> | void;
...
describe("KVCache", () => {
  ...
  describe("get", () => {
    it("returns cached value on success", async (ctx) => {
      ...
      expect(fakeCache.getCallsFor("get")[0].args[0]).toBe(
        "cached-kv:test/path",
      );
    });
    ...
    it("returns null and logs error on failure", async (ctx) => {
      ...
      expect((errors[0].error as Error).message).toBe("Cache unavailable");
    });
  });
  ...
  describe("set", () => {
    ...
    it("logs error on failure but does not throw", async (ctx) => {
      ...
      expect((errors[0].error as Error).message).toBe("Cache write failed");
    });
  });
  ...
  describe("invalidate", () => {
    ...
    it("succeeds on retry after initial failure", async (ctx) => {
      ...
      expect(errors.length).toBe(1);
      expect(errors[0].message).toContain("attempt 1/3");
      expect((errors[0].error as Error).message).toBe("Temporary failure");
    });
...
describe("encodeCacheKey", () => {
  ...
  baseIt("encodes unicode characters", () => {
    ...
    // Mixed
    expect(encodeCacheKey("path/유니코드/key")).toBe(
      "path/%EC%9C%A0%EB%8B%88%EC%BD%94%EB%93%9C/key",
    );
  });
  ...
  baseIt("handles typical cache key paths", () => {
    // Standard path
    expect(encodeCacheKey("cached-kv/app/users/123.value")).toBe(
      "cached-kv/app/users/123.value",
    );

    // Path with unicode key
    expect(encodeCacheKey("cached-kv/app/users/사용자.value")).toBe(
      "cached-kv/app/users/%EC%82%AC%EC%9A%A9%EC%9E%90.value",
    );

    // Path with spaces in key
    expect(encodeCacheKey("cached-kv/app/my key.value")).toBe(
      "cached-kv/app/my+key.value",
    );
  });
