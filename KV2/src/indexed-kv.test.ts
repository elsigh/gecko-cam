...
describe("TypedKV indexes", () => {
  ...
  function createStore(ctx: { kv: import("./cached-kv.js").KV2<TestMeta> }) {
    return ctx.kv.getStore<
      TestDoc,
      TestMeta,
      "bySlug" | "byStatus" | "byTag" | "byCreatedAt"
    >("doc/", {
      bySlug: {
        key: (doc) => doc.slug,
        unique: true,
  ...
  describe("unique index enforcement", () => {
    it("throws KVIndexConflictError on duplicate unique index key", async (ctx) => {
      ...
      expect((error as KVIndexConflictError).indexKey).toBe("hello");
    });
  ...
  describe("update index keys", () => {
    ...
    it("entry.update() maintains indexes", async (ctx) => {
      ...
      if (result.exists) {
        const value = await result.value;
        await result.update(
          { ...value, slug: "updated", status: "published" },
          { createdBy: "test", version: 2 },
        );
      }
  ...
  describe("prefix index queries", () => {
    ...
    it("prefix scan on unique index", async (ctx) => {
      ...
      const helloEntries: Array<[string, string]> = [];
      for await (const [key, entry] of store.entries({
        bySlug: { prefix: "hello" },
      })) {
  ...
  describe("error cases", () => {
    it("get throws for unknown index", async (ctx) => {
      ...
      try {
        ...
        await store.get({ nonexistent: "key" } as any);
      } catch (e) {
        error = e as Error;
      }
      expect(error).toBeDefined();
